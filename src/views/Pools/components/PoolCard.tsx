...
import { Button, IconButton, useModal, AddIcon, Image } from '@lydiafinance/uikit'
...
import OldElectrumTitle from './OldElectrumTitle'
...
const PoolCard: React.FC<HarvestProps> = ({ pool }) => {
  ...
  const isAvaxPool = poolCategory === PoolCategory.AVALANCHE
  const TranslateString = useI18n()
  const stakingTokenContract = useERC20(stakingToken.address ? getAddress(stakingToken.address) : '')
  const { account } = useWeb3React()
  const { onApprove } = useSousApprove(stakingTokenContract, sousId)
  const { onStake } = useSousStake(sousId, isAvaxPool)
  const { onUnstake } = useSousUnstake(sousId)
  const { onReward } = useSousHarvest(sousId, isAvaxPool)
  ...
  const isOldElectrum = stakingToken.symbol === tokens.electrum.symbol
  const accountHasStakedBalance = stakedBalance?.toNumber() > 0
  const needsApproval = !accountHasStakedBalance && !allowance.toNumber() && !isAvaxPool
  ...
  const poolImage = `${pool.earningToken.symbol}-${pool.stakingToken.symbol}.svg`.toLocaleLowerCase('en-US')
    ...
    <Card isActive={isCardActive} isFinished={isFinished && sousId !== 0}>
      ...
      <div style={{ padding: '24px' }}>
        <CardTitle isFinished={isFinished && sousId !== 0}>
          {isOldElectrum && '[OLD]'} {earningToken.symbol} {TranslateString(348, 'Pool')}
        </CardTitle>
        <div style={{ marginBottom: '8px', display: 'flex', alignItems: 'center' }}>
          <div style={{ flex: 1 }}>
            <Image src={`/images/pools/${poolImage}`} alt={earningToken.symbol} width={64} height={64} />
          </div>
          {account && harvest && !isOldElectrum && (
            <HarvestButton
