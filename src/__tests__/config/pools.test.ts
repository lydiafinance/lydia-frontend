...
describe('Config pools', () => {
  ...
  it.each(poolsToTest.filter((pool) => pool.earningToken.symbol !== 'AVAX'))(
  ...
  it.each(poolsToTest.filter((pool) => pool.stakingToken.symbol !== 'AVAX'))(
    'Pool %p has the correct staking token',
    async (pool) => {
      const contract = getSouschefContract(pool.sousId)
      const stakingTokenAddress = await contract.methods.electrum().call()
